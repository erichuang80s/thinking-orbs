...
import PlayPauseToggle from './PlayPauseToggle.vue';
...
const snippet = computed(() => buildSnippet(state.value, size.value, props.speed));
const fillPct = computed(() => ((props.speed - SPEED_MIN) / (SPEED_MAX - SPEED_MIN)) * 100);

function onSpeedInput(e: Event) {
  emit('update:speed', Number((e.target as HTMLInputElement).value));
}
</script>
...
<template>
  <section class="w-full flex flex-col gap-1.5 mb-12" aria-label="Interactive playground">
    ...
    <div class="flex flex-col gap-4 bg-(--panel-bg) rounded-[10px] p-4">
      <div class="flex items-end gap-6 max-sm:flex-col max-sm:items-stretch max-sm:gap-4">
        <div class="flex flex-col gap-[9px] min-w-0" role="radiogroup" aria-label="Orb state">
      ...
      <div class="flex items-end gap-6 max-sm:flex-col max-sm:items-stretch max-sm:gap-4">
        ...
        <div class="flex flex-col gap-[9px] min-w-[100px] w-[140px] max-sm:w-full">
          ...
          <div
            class="strength-track relative w-full h-9 rounded-lg bg-(--strength-bg) shadow-(--strength-shadow) overflow-hidden cursor-grab active:cursor-grabbing hover:bg-(--strength-hover)"
          >
            <div
            ...
            />
            <span
              class="absolute top-0 left-[11px] h-full flex items-center text-[11px] font-normal leading-[14px] text-(--text-muted) whitespace-nowrap pointer-events-none z-[1]"
              >{{ (speed / 100).toFixed(2) }}×</span
            >
            <input
    ...
    <div
      class="relative w-full min-h-[304px] rounded-[10px] bg-(--surface) flex flex-col items-center justify-center p-12 gap-6 max-sm:p-6"
    >
      <ThinkingOrb
        :key="`${state}-${size}`"
        :state="state"
      ...
      <PlayPauseToggle
        :playing="!paused"
        class="max-sm:absolute max-sm:bottom-6 max-sm:left-1/2 max-sm:-translate-x-1/2"
        @toggle="paused = !paused"
      />
    </div>

    <div
      class="flex items-start h-auto bg-(--code-bg) rounded-[10px] py-1.5 pr-10 pl-3 overflow-hidden relative max-sm:hidden"
    >
      <code
        class="font-[Roboto_Mono,monospace] text-sm leading-[22px] text-(--code-text) whitespace-pre overflow-x-auto min-w-0 flex-1"
        >{{ snippet }}</code
      >
      <CopyButton :get-text="() => snippet" />
    </div>
  </section>
</template>
